...
def compute_laplace_noise_parameters(sensitivity, epsilon):
    b = sensitivity / 2 * epsilon
    return b

def compute_laplace_noise_parameters_new(sensitivity, epsilon):
    b = sensitivity / epsilon
    return b

def compute_gaussian_noise_parameters(sensitivity, epsilon):
    sigma2 = (sensitivity ** 2) / (4 * epsilon)
    return sigma2

def compute_gaussian_noise_parameters_new(sensitivity, epsilon):
    sigma2 = (sensitivity ** 2) / (2 * epsilon)
    return sigma2

def add_noise_rrldp(tensor, epsilon, sensitivity, noise_type='laplace'):
    device = tensor.device

    # # 判断 tensor 是否为全零张量
    # if torch.all(tensor == 0):
    #     return tensor  # 如果为全零张量，直接返回

    noise_tensor = torch.zeros_like(tensor, device=device)
    if noise_type == 'laplace':
        b = compute_laplace_noise_parameters_new(sensitivity, epsilon)
        # p = np.exp(epsilon - (sensitivity / b)) / (2 * b + np.exp(epsilon - (sensitivity / b)))
        p = np.exp(epsilon - (torch.numel(tensor) * sensitivity / b)) / (2 * b + np.exp(epsilon - (torch.numel(tensor) * sensitivity / b)))

        rnd = torch.rand(tensor.shape, device=device)
        mask = rnd > p
        # print("laplace p:", p)
        # print("rrldp laplace mask: ", mask)
        noise_tensor = noise_tensor.float()
        noise = torch.distributions.Laplace(0, b).sample(tensor.shape).to(device)
        noise_tensor[mask] = noise[mask]

    elif noise_type == 'gaussian':
        sigma2 = compute_gaussian_noise_parameters_new(sensitivity, epsilon)
        # sigma2 放在根号里面
        # p = np.exp(epsilon - (sensitivity ** 2 / (2 * sigma2))) / (np.sqrt(sigma2 * 2 * np.pi) + np.exp(epsilon - (sensitivity ** 2 / (2 * sigma2))))
        p = np.exp(epsilon - (torch.numel(tensor) * sensitivity ** 2 / (2 * sigma2))) / (np.sqrt(sigma2 * 2 * np.pi) + np.exp(epsilon - (torch.numel(tensor) * sensitivity ** 2 / (2 * sigma2))))

        rnd = torch.rand(tensor.shape, device=device)
        mask = rnd > p
        # print("gaussian p: ", p)
        # print("rrldp gaussian mask: ", mask)

        noise = torch.distributions.Normal(0, sigma2).sample(tensor.shape).to(tensor.device)
        noise_tensor[mask] = noise[mask]

    return tensor + noise_tensor

def add_weighted_noise_rrldp(tensor, epsilon, sensitivity, weight_matrix, noise_type='laplace'):
    device = tensor.device
    noise_tensor = torch.zeros_like(tensor, device=device)

    if noise_type == 'laplace':
        b = compute_laplace_noise_parameters_new(sensitivity, epsilon)
        # p_base = np.exp(epsilon - (sensitivity / b)) / (2 * b + np.exp(epsilon - (sensitivity / b)))
        p_base = np.exp(epsilon - (torch.numel(tensor) * sensitivity / b)) / (2 * b + np.exp(epsilon - (torch.numel(tensor) * sensitivity / b)))
    elif noise_type == 'gaussian':
        sigma2 = compute_gaussian_noise_parameters_new(sensitivity, epsilon)
        # p_base = np.exp(epsilon - (sensitivity ** 2 / (2 * sigma2))) / (np.sqrt(sigma2 * 2 * np.pi) + np.exp(epsilon - (sensitivity ** 2 / (2 * sigma2))))
        p_base = np.exp(epsilon - (torch.numel(tensor) * sensitivity ** 2 / (2 * sigma2))) / (np.sqrt(sigma2 * 2 * np.pi) + np.exp(epsilon - (torch.numel(tensor) * sensitivity ** 2 / (2 * sigma2))))

    weight_matrix = torch.tensor(weight_matrix).to(device)
    weight_matrix = torch.stack([weight_matrix, weight_matrix, weight_matrix], dim=0)
    p = (1 - weight_matrix) * p_base
...
if __name__ == "__main__":
    ...
    # example_tensor = torch.tensor([[0.1, 0.2], [0.3, 0.4]])
    example_tensor = (torch.rand(3, 2, 2) * 2) - 1
    weight_matrix = torch.tensor([[0.1, 0.2], [0.3, 0.4]])
    print("原始tensor:")
    print(example_tensor)
    print("原始tensor shape: ", example_tensor.shape)

    mvg = mvg_mechanism(example_tensor, 1, 1, np.random.rand(example_tensor.shape[0] * example_tensor.shape[2]) / np.sum(np.random.rand(example_tensor.shape[0] * example_tensor.shape[2])))
    print(mvg)

    # scale = (torch.numel() * sensitivity) / epsilon
    laplace_noised_tensor = add_laplace_noise(example_tensor, scale=1.0)
    print("\n添加Laplace噪声后的tensor:")
    print(laplace_noised_tensor)

    # mean = 0
    std = (torch.numel(example_tensor) * sensitivity**2) / (2 * epsilon)
    gaussian_noised_tensor = add_gaussian_noise(example_tensor, mean=0.0, std=std)
    print("\n添加Gaussian噪声后的tensor:")
    print(gaussian_noised_tensor)

    #
    rrldp_laplace_noised_tensor = add_noise_rrldp(example_tensor, epsilon, sensitivity, noise_type='laplace')
    print("\nRRLDP Laplace:")
    print(rrldp_laplace_noised_tensor)

    rrldp_gaussian_noised_tensor = add_noise_rrldp(example_tensor, epsilon, sensitivity, noise_type='gaussian')
    print("\nRRLDP Gaussian:")
    print(rrldp_gaussian_noised_tensor)

    rrldp_weight_laplace_noised_tensor = add_weighted_noise_rrldp(example_tensor, epsilon, sensitivity, weight_matrix, noise_type='laplace')
    print("\n添加加权RRLDP Laplace噪声后的tensor:")
    print(rrldp_weight_laplace_noised_tensor)

    rrldp_weight_gaussian_noised_tensor = add_weighted_noise_rrldp(example_tensor, epsilon, sensitivity, weight_matrix, noise_type='gaussian')
    print("\n添加加权RRLDP Gaussian噪声后的tensor:")
    print(rrldp_weight_gaussian_noised_tensor)

    # perturbed_tensor = perturb_tensor_krr(example_tensor, epsilon=epsilon)
    perturbed_tensor = perturb_tensor_krr_new(example_tensor, epsilon=epsilon)
    print("\npure tensor:")
    print(example_tensor)
    print("\nKRR tensor:")
