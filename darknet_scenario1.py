...
output_size = 128
num_classes = 2
deep_model = FCNClassifier(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, output_size=output_size, num_classes=num_classes, dropout_rate=0.3, use_batch_norm=False)
deep_model = deep_model.to(device)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(deep_model.parameters(), lr=0.005)

# 将train_loader展开为tensor
features = []
targets = []
...
for feature in features:
    if config["noise_type"] == "none":
        # noisy_data = feature
        noisy_data.append(feature)
    elif config["noise_type"] == "privacy":
        if config["privacy_function"] == "rrldp_laplace":
            noisy_data.append(add_noise_rrldp(feature.to(device), epsilon=config["epsilon"],
            ...
            import math
            sensitivity = 2 * math.sqrt(torch.numel(feature))
            noisy_data.append(add_optimized_tensor_noise_Yang(feature.to(device), epsilon=config["epsilon"],
                                                           sigma=10 ** (-5), sensitivity=sensitivity))
        elif config["privacy_function"] == "laplace":
            import math
            # sensitivity = 2 * math.sqrt(torch.numel(sample))
            scale = (torch.numel(feature) * config["sensitivity"]) / config["epsilon"]
            noisy_data = add_laplace_noise(feature.to(device), scale=scale)
        elif config["privacy_function"] == "gaussian":
            import math
            std = (torch.numel(feature) * config["sensitivity"] ** 2) / (2 * config["epsilon"])
            # sensitivity = 2 * math.sqrt(torch.numel(sample))
            # std = (sensitivity * torch.log(1 / torch.tensor(1e-5))) / config["epsilon"]
            noisy_data = add_gaussian_noise(feature.to(device), mean=0.0, std=std)
        elif config["privacy_function"] == "mvg_mechanism":
            noisy_data.append(mvg_mechanism_2d(X=feature.unsqueeze(0).to(device), epsilon=config["epsilon"], delta=10 ** (-5),
                                         theta=[0.55 / 2, 0.55 / 2] + [0.45 / (feature.shape[0] - 2)] * (feature.shape[0] - 2)))

all_features = []
all_targets = []
# all_features.append(noisy_data)
all_targets.append(targets)
all_features = torch.stack(noisy_data).to(device)
...
train_model(deep_model, train_loader, criterion, optimizer, num_epochs=25, patience=3, device=device)
